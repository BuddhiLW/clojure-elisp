...
the runtime library `clel.el` must be loadable. `cider-cljel-mode` loads it from
...
| Runtime | `resources/clojure-elisp/clel.el` (the Emacs package `clel`) | 55+ Elisp functions implementing Clojure semantics |
...
`bundle-runtime!` writes the runtime, `clel.el`, out of the dependency, so
...
Compiled `.el` files require the runtime library, the Emacs package `clel`
(`resources/clojure-elisp/clel.el`, compiled from `runtime.cljel` by
`make runtime`). Install it via MELPA (once available):
...
Before 0.8.0 the runtime was `clojure-elisp-runtime.el`, providing the feature
`clojure-elisp-runtime`. It was renamed because MELPA requires every definition
in a package to start with the package's name, and the runtime's all start
with `clel`. Code compiled by 0.8.0 requires `clel`; recompile code compiled
