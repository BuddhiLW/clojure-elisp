...
* Backlog
** TODO Implement destructuring in fn/defn parameters
:PROPERTIES:
:ID: clel-003
:PRIORITY: A
:END:
Once let destructuring works, extend to function parameters.
Support & rest args: (defn foo [a b & rest] ...)

** TODO Add throw support
:PROPERTIES:
:ID: clel-005
:PRIORITY: A
:END:
Implement (throw (ex-info "msg" {:data x}))
Maps to Elisp (signal 'error ...)

** TODO Support multi-arity functions
:PROPERTIES:
:ID: clel-006
:PRIORITY: A
:END:
Handle (defn foo ([x] ...) ([x y] ...) ([x y z] ...))
Emit as single defun with arity dispatch via cl-case on arg count

** TODO Implement case special form
:PROPERTIES:
:ID: clel-007
:PRIORITY: B
:END:
...
** TODO Implement letfn for local functions
:PROPERTIES:
:ID: clel-008
:PRIORITY: B
:END:
(letfn [(foo [x] ...) (bar [y] ...)] body)
Emit as cl-labels
...
** TODO Handle and/or as special forms
:PROPERTIES:
:ID: clel-010
:PRIORITY: B
:END:
Currently in core-fn-mapping but they need short-circuit evaluation.
Treat as special forms that emit proper and/or Elisp forms.
